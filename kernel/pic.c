...
void pic_init(void)
{
     ...
     * Master: IRQ0 -> 0x20, IRQ7 -> 0x27
     * Slave: IRQ8  -> 0x28, ... IRQ15 -> 0x2F
     */
     ...
     * Master gets a bit mask: 00000100b = 0x04
     * Slave gets its cascade identity: IRQ2 = 0x02
     */
...
void pic_unmask_irq(unsigned int irq)
{
    ...
    //1 = IRQ disabled, 0 = IRQ enabled, Clear the bit corresponding to this IRQ.
    mask &= ~(1 << irq);
